...
function boxwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('boxwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'boxwp'), 'priority' => 10, ));
    endif;

    $wp_customize->get_section( 'title_tagline' )->panel = 'boxwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'boxwp_main_options_panel';
    ...
    boxwp_getting_started($wp_customize);
    boxwp_header_options($wp_customize);
    boxwp_post_options($wp_customize);
    boxwp_social_profiles($wp_customize);
    boxwp_footer_options($wp_customize);
    boxwp_other_options($wp_customize);
    boxwp_recomm_plugin_options($wp_customize);
    boxwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'boxwp_register_theme_customizer' );

function boxwp_customizer_js_scripts() {
    wp_enqueue_script('boxwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'boxwp_customizer_js_scripts' );