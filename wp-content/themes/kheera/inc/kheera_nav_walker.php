...
*/
	...
	class Kheera_Nav_Walker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = Array() ) {
			...
			if(isset($this->currentItem)){
				if(in_array('menu-item-has-children',$this->currentItem->classes)){
					$output .= "\n" . $indent . '<ul id="collapse-menu-item-'.$this->currentItem->ID.'" class="dropdown-menu">' . "\n";
				}
				else{
			...
			if(isset($item->classes)){
				if($depth > 0){ 
					if(in_array('menu-item-has-children',$item->classes)){
						...
						$output .= '<span class="submenu" data-toggle="collapse" data-target="#collapse-menu-item-'.$item->ID.'" ><i class="fas fa-angle-down" aria-hidden="true"></i></span>';
					}
					else{
						$output .= "\n" . $indent . "<li class='" .  implode(" ", $item->classes) . "'>";
						$output .= '<a itemprop="url" href="' . $item->url . '">';
